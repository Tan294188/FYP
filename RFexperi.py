...
models =  RandomForestClassifier(n_estimators=175, random_state=42, max_depth=None)
X = df.drop('y', axis=1)  
y = df['y']
...
for a in {False}:
    for b in {None}:
        for c in range (50,155,5):
            print(a,b,c)
            models =  RandomForestClassifier(n_estimators=c, random_state=42, max_depth=b, bootstrap=a)
