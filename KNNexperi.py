...
df = df[~(df['pdays'] == -1) ]
df = df[~(df['balance'] < 0)]
models = KNeighborsClassifier(n_neighbors=8,weights = 'distance', metric = 'manhanttan')
...
for a in range(2,11,1):
    for b in {"uniform","distance"}:
        for c in {"euclidean","manhattan","minkowski"}:
            print(a,b,c)
            models = KNeighborsClassifier(n_neighbors=a, weights=b, metric=c)
